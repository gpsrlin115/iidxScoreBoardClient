...
export default function Profile() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-6 sm:px-8 sm:py-8">
      <p className="mb-3 font-mono text-[10px] uppercase tracking-[.2em] text-label">profile & account</p>
      <h1 className="mb-3 text-2xl font-medium text-ink">프로필</h1>
      <p className="mb-8 text-sm leading-6 text-muted">표시 이름, IIDX-ID, 보안과 로그인 수단을 관리합니다.</p>
      <GoogleCallbackError />
      ...
      <ProfileAvatar user={user} onUserChange={setUser} />
      <ProfileDetailsForm user={user} onUserChange={setUser} />
      ...
      <DataActions iidxId={user?.iidxId} />
      ...
      {methods && <>
        ...
        <section className="border border-line-strong bg-panel p-5 sm:p-7" aria-labelledby="password-title">
          ...
          {methods.passwordEnabled ? <>
            <p className="text-sm leading-6 text-muted">현재 비밀번호를 확인한 뒤 새 비밀번호로 변경합니다. 변경하면 다른 기기의 로그인은 종료됩니다.</p>
            <PasswordChangeForm />
          </> : <>
